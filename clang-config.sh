...
case $(uname -i) in
    ppc64le)
        export CXXFLAGS="-fPIC -mcpu=native -mtune=native -ffast-math -std=c++17 -D__STDC_WANT_LIB_EXT2__=1 "
	export OCT_ARCH_FLAGS="-mcpu=native,-mtune=native"
        export LIB_DIR_NAME=lib64
        export LIBHPX=lib64
        ;;
    x86_64)
        export CXXFLAGS="-fPIC -march=native -ffast-math -std=c++17 -D__STDC_WANT_LIB_EXT2__=1 "
